<template>
  <div class="edit-stl-page">
    <div class="toolbar">
      ...
      <button
        class="tool-button"
        :class="{ active: activeTool === 'boundary' }"
        type="button"
        @click="setActiveTool('boundary')"
      >
        移动边界
      </button>
      <button
        class="tool-button"
        :class="{ active: workflowMode === 'paintThenBoundary' }"
        type="button"
        @click="setWorkflowMode('paintThenBoundary')"
      >
        先涂色再边界
      </button>
      <button
        class="tool-button"
        :class="{ active: workflowMode === 'boundaryOnly' }"
        type="button"
        @click="setWorkflowMode('boundaryOnly')"
      >
        直接调边界
      </button>
      <button
        class="tool-button"
        :class="{ active: brushMode === 'tooth' }"
        :disabled="activeTool !== 'paint'"
        type="button"
        @click="brushMode = 'tooth'"
      >
        牙体
      </button>
      <button
        class="tool-button"
        :class="{ active: brushMode === 'gingiva' }"
        :disabled="activeTool !== 'paint'"
        type="button"
        @click="brushMode = 'gingiva'"
      >
        牙龈
      </button>
      <button
      ...
      <label class="tool-item">
        ...
        <input
          ...
          class="tooth-input"
          :disabled="activeTool !== 'paint' || brushMode !== 'tooth'"
          type="number"
          min="11"
          max="48"
        />
      ...
      <label class="tool-item">
        <span>原 FDI</span>
        <input v-model.number="sourceFdi" class="tooth-input" type="number" min="11" max="48" />
      </label>
      <label class="tool-item">
        <span>新 FDI</span>
        <input v-model.number="targetFdi" class="tooth-input" type="number" min="11" max="48" />
      </label>
...
const activeTool = ref<ToolMode>('paint')
const workflowMode = ref<WorkflowMode>('paintThenBoundary')
...
function buildToothLabelGroup(geometry: THREE.BufferGeometry, labels: number[]) {
  ...
  Array.from(sums.entries())
    ...
    .forEach(([label, item]) => {
      ...
      if (!sprite) return
      sprite.userData.isFdiLabel = true
      sprite.userData.fdi = label
...
function syncVisibility() {
  ...
  const boundaryVisible = showBoundaries.value && activeTool.value === 'boundary'
  if (boundaryGroups.upper) boundaryGroups.upper.visible = showUpper.value && boundaryVisible
  if (boundaryGroups.lower) boundaryGroups.lower.visible = showLower.value && boundaryVisible
}
...
function setActiveTool(tool: ToolMode) {
  ...
  statusText.value =
    tool === 'paint'
      ? '涂色模式：选择牙体、牙龈或擦除，并拖动模型修正大范围分割。'
      : '移动边界模式：大范围分割基本准确时，拖动边界控制点微调。'
}

function setWorkflowMode(mode: WorkflowMode) {
  workflowMode.value = mode
  setActiveTool(mode === 'paintThenBoundary' ? 'paint' : 'boundary')
}
...
function isValidFdi(value: number | null) {
  return Number.isInteger(value) && !!value && value >= 11 && value <= 48
}

function meshHasLabel(mesh: THREE.Mesh, fdi: number) {
...
function changeFdiOnMesh(mesh: THREE.Mesh, fromFdi: number, toFdi: number) {
  ...
  if (meshHasLabel(mesh, toFdi)) {
    statusText.value = `FDI ${toFdi} 已存在，已取消修改以避免合并两颗牙。`
    return 0
  }

  let changedVertexCount = 0
  for (let index = 0; index < labels.length; index++) {
    if (labels[index] !== fromFdi) continue
    labels[index] = toFdi
    changedVertexCount += 1
  }

  if (!changedVertexCount) return 0
...
function applyFdiChange() {
  ...
  if (!isValidFdi(fromFdi) || !isValidFdi(toFdi)) {
    statusText.value = '请输入有效的原 FDI 和新 FDI。'
    return
  }

  if (fromFdi === toFdi) {
    statusText.value = '原 FDI 和新 FDI 相同，无需修改。'
    return
  }

  const matchedMeshes = getVisibleMeshes().filter((mesh) => meshHasLabel(mesh, fromFdi))
  if (!matchedMeshes.length) {
    statusText.value = `未找到 FDI ${fromFdi} 对应的牙齿。`
    return
  }

  let changedVertexCount = 0
  matchedMeshes.forEach((mesh) => {
    changedVertexCount += changeFdiOnMesh(mesh, fromFdi, toFdi)
  })

  if (!changedVertexCount) return
...
function updateBrushIndicator(hit?: THREE.Intersection | null) {
  if (!brushIndicator || activeTool.value !== 'paint') {
    hideBrushIndicator()
    return
  }
  if (!hit) {
  ...
  if (brushIndicator.material instanceof THREE.MeshBasicMaterial) {
    brushIndicator.material.color.copy(getBrushColor())
  }
}

function createBrushIndicator() {
  if (!scene || brushIndicator) return
  ...
  brushIndicator.name = 'paint-brush-indicator'
...
function getMeshIntersect(event: PointerEvent) {
  ...
  return raycaster.intersectObjects(getVisibleMeshes(), false)[0] ?? null
}

function refreshBoundaryGroupFromLabels(mesh: THREE.Mesh) {
  const jaw = mesh.userData.jaw as JawType | undefined
  const labels = meshLabelMap.get(mesh)
...
function onPointerMove(event: PointerEvent) {
  ...
  if (activeTool.value === 'paint') {
    ...
    updateBrushIndicator(getMeshIntersect(event))
    return
  }
...
onUnmounted(() => {
  ...
  controls?.dispose()
  disposeObject(brushIndicator ?? undefined)
...
.tool-item input[type='range']:disabled {
  cursor: not-allowed;
  opacity: 0.55;
}
...
.viewer :deep(canvas.painting-labels) {
  cursor: crosshair;
}
</style>
