 ...
 */
export function extractBoltonData(diagnosisData, type) {
  if (diagnosisData.length == 0) {
    return null;
  }

  const pathologyResults = diagnosisData as Array<Record<string, unknown>>;
  const boltonResult = pathologyResults.find(result => result.task_name === type);
