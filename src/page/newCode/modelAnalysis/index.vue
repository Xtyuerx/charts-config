...
const handleSave = () => {
  ...
  if (currentTaskName === 'midline-deviation') {
  ...
  } else if (currentTaskName === 'arch-symmetry') {
    // 🔥 直接从 analysisData 中获取原始的中线点位数据
    const originalMidlinePoints = getOriginalMidlinePointsFromAnalysisData();

    // 合并更新后的点位数据和原始的中线点位数据
    const completeData = {
      teeth_points: updatedData, // 更新后的牙齿点位数据
      ...originalMidlinePoints, // 原始的 upper_midline_points 和 lower_midline_points
    };
    ...
    params = {
      ...
      upperStlUrl: JSON.parse(detailParams.value.stlFileList).STL_UPPER_DENTAL_ARCH[0].fileUrl,
      lowerStlUrl: JSON.parse(detailParams.value.stlFileList).STL_LOWER_DENTAL_ARCH[0].fileUrl,
      // 🔥 保存完整的数据（包含更新后的点位和原始的中线点位）
      updatedJson: JSON.stringify(completeData),
    };
  } else {
...
// 🔥 新增：从 analysisData 中获取原始的中线点位数据
const getOriginalMidlinePointsFromAnalysisData = () => {
  const result = {}; // 保持 const
  // 遍历 analysisData 查找中线点位数据
  if (analysisData.value && Array.isArray(analysisData.value)) {
    analysisData.value.forEach(taskData => {
      if (taskData.task_name === 'arch-symmetry') {
        // 使用对象属性赋值而不是重新赋值整个对象
        result.upper_midline_points = taskData.diagnosis_result.upper_midline_points;
        result.lower_midline_points = taskData.diagnosis_result.lower_midline_points;
      }
    });
  }
  return result;
