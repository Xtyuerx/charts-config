...
export class AnalysisService {
  ...
  switchAnalysis(taskName: string): boolean {
    ...
    if (strategy instanceof LowerCurveAnalysisStrategy) {
      this.registerLowerCurveDraggableObjects(strategy);
    }
    if (strategy instanceof UpperCurveAnalysisStrategy) {
      this.registerUpperCurveDraggableObjects(strategy);
    }
   ...
   * 注册上颌曲线分析的可拖拽对象
   */
  private registerUpperCurveDraggableObjects(strategy: UpperCurveAnalysisStrategy): void {
    const draggableObjects = strategy.getDraggableObjects();
    if (draggableObjects.length > 0) {
      console.log(`🎯 上颌曲线分析 - 注册${draggableObjects.length}个可拖拽点位`);
      const sceneManager = SceneManager.getInstance();

      draggableObjects.forEach(obj => {
        sceneManager.addDraggableObject(obj);
        obj.userData.draggable = true;
        obj.userData.isUpperCurvePoint = true; // 标记为上颌曲线分析点位
   ...
   * 注册下颌曲线分析的可拖拽对象
   */
  private registerLowerCurveDraggableObjects(strategy: LowerCurveAnalysisStrategy): void {
    const draggableObjects = strategy.getDraggableObjects();
    if (draggableObjects.length > 0) {
      console.log(`🎯 下颌曲线分析 - 注册${draggableObjects.length}个可拖拽点位`);
      ...
      draggableObjects.forEach(obj => {
        sceneManager.addDraggableObject(obj);
        obj.userData.draggable = true;
        obj.userData.isLowerCurvePoint = true; // 标记为下颌曲线分析点位
        obj.userData.strategy = strategy; // 保存策略引用
      });

      // 初始化拖拽控制
      sceneManager.setupDragControls();
      console.log(`✅ 下颌曲线分析可拖拽对象注册完成`);
    }
  }
  /**
   * 注册牙弓对称性分析的可拖拽对象
  ...
  getCurrentUpdatedPoints(): any[] | any | null {
    ...
    if (this.currentStrategy instanceof LowerCurveAnalysisStrategy) {
      return this.currentStrategy.getUpdatedPoints();
    }
    if (this.currentStrategy instanceof UpperCurveAnalysisStrategy) {
      return this.currentStrategy.getUpdatedPoints();
    }
    return null;
  }
