...
export class ToothGapAnalysisStrategy extends BaseAnalysisStrategy {
  ...
  private renderGap(
    ...
    // 🔥 修改：创建可拖拽的点位球体
    const sphere1 = this.createDraggablePointSphere(pos1, pointColor1, smallerFdi, point1);
    const sphere2 = this.createDraggablePointSphere(pos2, pointColor2, largerFdi, point2);
   ...
   * 创建可拖拽的点位球体
   */
  private createDraggablePointSphere(
    position: THREE.Vector3,
    color: number,
    fdi: number,
    originalPoint: import('../types').ToothPoint,
  ): THREE.Mesh {
    ...
    sphere.userData = {
      fdi: fdi,
      pointType: originalPoint.type,
      pointTypeCn: originalPoint.type_cn,
      originalPosition: position.clone(),
      originalPoint: originalPoint, // 保存原始点位数据
      isDraggable: true,
    };
  ...
  private createPointSphere(position: THREE.Vector3, color: number, fdi: number): THREE.Mesh {
    ...
    sphere.userData = {
      fdi: fdi,
      pointType: fdi < 30 ? 'boundary_distal' : 'boundary_mesial', // 根据FDI判断点位类型
      pointTypeCn: fdi < 30 ? '远中边缘' : '近中边缘',
      originalPosition: position.clone(),
      isDraggable: true,
    };
