...
export class OverbiteAnalysisStrategy extends BaseAnalysisStrategy {
  ...
  private createColoredPointMarkers(
    ...
    midlinePoints.forEach((point, index) => {
      ...
      console.log(nearestSamplePoint, 'nearestSamplePoint11111111111111');
  ...
  protected formatMeasurements(measurements: Record<string, unknown>): MeasurementGroup[] {
    const H_total = (measurements.H_total as number) || 0;
    const H_overlap = (measurements.H_overlap as number) || 0;
    const severity = (measurements.severity as string) || '正常';

    return [
      {
        groupName: '覆𬌗测量',
        groupResult: severity,
        children: [
          {
            name: '下前牙牙冠高度（mm）',
            value: `${H_total}mm`,
          ...
          {
            name: '覆盖率',
            value: `${(H_overlap / H_total).toFixed(2)}%`,
          },
        ],
      },
    ];
