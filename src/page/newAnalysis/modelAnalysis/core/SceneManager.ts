...
export class SceneManager {
  ...
  updateView(
    ...
    const { frontDirection, leftRightDirection, upDirection, targetPoint } = this.modelOrientation
    ...
    switch (viewKey) {
      ...
      case 'right': {
        ...
        cameraPosition = viewTarget.clone().add(leftRightDirection.clone().multiplyScalar(distance))
        break
      }
  ...
  adjustCameraToFaceFrontTeeth(toothCenters: { fdi: number; center: THREE.Vector3 }[]): void {
    ...
    if (tooth16 && tooth26) {
      ...
      const tooth16to26 = new THREE.Vector3().subVectors(tooth26.center, tooth16.center).normalize()
